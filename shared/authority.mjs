/**
 * The authority — the whole game simulation, with no transport and no Node.
 *
 * This file is deliberately plain ESM with zero imports. That is what lets the
 * SAME simulation run in three places: a Node process behind `ws` for
 * self-hosting, a Cloudflare Durable Object at the edge for free hosted rooms,
 * and anything else later. The relay was previously fused to `ws` in six
 * places, which meant "run it somewhere else" implied a rewrite.
 *
 * The host provides a transport and drives the clock:
 *
 *   attach({ send, log })   // send(connId, msg) delivers one message
 *   handleMessage(id, msg)  // an inbound message from a connection
 *   leave(id)               // a connection went away
 *   step(dt)                // advance the world; call at HZ
 *
 * Everything else in here is the game, unchanged.
 */
...
// Transport is injected by the host: Node `ws`, a Durable Object, or anything
// else. Nothing below this line knows which one it is running on.
let sendTo = (_id, _msg) => {};
let log = (..._a) => {};

/**
 * Install the host's transport. Call once before anything else.
 * @param {{ send(id: string, msg: unknown): void, log?: (...a: unknown[]) => void }} host
 */
...
/** Snapshot for a host that wants to report status. */
export function stats() {
  return {
    players: players.size,
    molochs: molochs.size,
    declarations: hypers.size,
    pressure: Number(world.molochPressure.toFixed(3)),
    banished: world.banished,
    seed: world.seed,
  };
}

const send = (id, msg) => sendTo(id, msg);
...
function pushChat(from, text, kind = 'say') {
  // `at`, not `t`: the wire envelope uses `t` for the message type, and
  // spreading an entry with its own `t` silently overwrote the discriminant —
  // chat arrived typed as a unix timestamp and clients had to sniff for it.
  const entry = { from, text, kind, at: Date.now() };
  chat.push(entry);
  ...
  for (const [uid, m] of molochs) {
    ...
    for (const [pid, p] of players) {
      if (Math.hypot(p.x - m.x, p.z - m.z) < 8) {
        send(pid, { t: 'drain', amount: dt * 2.2, uid: m.uid });
      }
    }
  }
}
...
/**
 * One message from one connection. `id` is whatever the host uses to address a
 * connection; the authority only requires that it is stable and unique.
 */
export function handleMessage(id, m) {
  if (!m || typeof m.t !== 'string') return;
  {
    switch (m.t) {
      case 'hello': {
        ...
        log(`+ ${m.name}${m.agent ? ' [agent]' : ''} (${id}) — ${players.size} online`);
        break;
      }
      ...
      case 'hyperstition': {
        ...
        if (def.needsMoloch) {
          ...
          if (!target || bd > 120) {
            send(id, { t: 'denied', why: 'That one is spoken at a Moloch, and none is within 120m.' });
            return;
          }
        }
      ...
      case 'align': {
        ...
        if (h.contributors.has(id)) {
          send(id, { t: 'denied', why: 'You have already aligned with that Hyperobject.' });
          return;
        }
        const d = Math.hypot(h.x - p.x, h.z - p.z);
        if (d > 60) {
          send(id, { t: 'denied', why: `Too far to align (${Math.round(d)}m, need 60m).` });
          return;
        }
      ...
      case 'attack': {
        ...
        send(id, { t: 'denied', why: 'Moloch does not answer to force. He is a shape a group of raptors makes when none of them can trust the others.' });
        break;
      }
    }
  }
...
export function leave(id) {
  {
    ...
    if (p) log(`- ${p.name} (${id}) — ${players.size} online`);
  }
}
...
/** Advance the world. The host decides how often to call this. */
export function step(dt = 1 / HZ) {
  {
